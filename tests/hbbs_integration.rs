...
#[tokio::test]
async fn test_punch_hole_peer_not_found() {
    let server = hbbs::RendezvousServer::start_test("")
        .await
        .expect("Failed to start test server");
    let addr = format!("127.0.0.1:{}", server.udp_port);
    ...
    assert!(resp.is_some());
    let resp = resp.unwrap();
    assert!(resp.has_punch_hole_response());
    assert_eq!(
        resp.punch_hole_response().failure,
...
#[tokio::test]
async fn test_register_then_punch_hole() {
    let server = hbbs::RendezvousServer::start_test("")
        .await
        .expect("Failed to start test server");
    let addr = format!("127.0.0.1:{}", server.udp_port);

    let socket_b = UdpSocket::bind("0.0.0.0:0").await.unwrap();
    let mut msg = RendezvousMessage::new();
    msg.set_register_peer(RegisterPeer {
        id: "peer_b_1".to_owned(),
        ..Default::default()
    });
    let resp = send_recv_udp(&socket_b, &msg, &addr).await.unwrap();
    assert!(resp.has_register_peer_response());
    ...
    let socket_a = UdpSocket::bind("0.0.0.0:0").await.unwrap();
    let mut msg = RendezvousMessage::new();
    msg.set_punch_hole_request(PunchHoleRequest {
        id: "peer_b_1".to_owned(),
        ..Default::default()
    ...
    let mut found = false;
    for _ in 0..5 {
        match timeout(Duration::from_secs(3), socket_b.recv_from(&mut buf)).await {
            Ok(Ok((n, _))) => {
                if let Ok(resp) = RendezvousMessage::parse_from_bytes(&buf[..n]) {
    ...
    assert!(found, "Peer B should get PunchHole or FetchLocalAddr");
